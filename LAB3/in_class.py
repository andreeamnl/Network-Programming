...
def fetch_and_parse(current, urls, data, max):
    ...
    try:
        ...
        if current == max or current >= len(urls)-1:
            return data
        else:
            
            fetch_and_parse(current+1, urls, data, max)
    except Exception as e:
        print(e)

init = fetch_and_parse(0, urls, data, 10)
